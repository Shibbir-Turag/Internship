...
"""
SCRIPT 03 — THIRD TRIAL 70 SQUARE CELL GRID DETECTION

Purpose
-------
Detect the fixed 7 x 10 = 70 planting-cell layout from the cropped D/RGB
tray images created by Third Trial Script 01.
...
Why square zones?
-----------------
The square cell zone is larger than the visible cup. This allows a seedling
...
Trial 3 note
------------
Day 7 may include cells where plants were eaten by bugs. That issue is not
handled in this grid script. It will be handled later in the growth-rate
comparison stage by keeping both observed and adjusted values.
"""
...
def candidate_evidence(
    ...
    - a lighter white rim or foam around the outside

    This rejects false Hough detections on blank tray/background areas.
    """
...
def detect_pot_candidates(rgb: np.ndarray):
    ...
    for param2 in [58, 52, 46, 40, 34, 28, 24]:
...
def get_manual_corner_centres(
    ...
    axis.set_title(
        title
        + "\nClick cell centres: Cell 1 → Cell 10 → Cell 70 → Cell 61"
...
def save_square_grid_overlay(
    ...
    for cell in cells:
        ...
        text_x = max(
            1,
            int(round(cell["x"] - median_side * 0.12)),
        )
    ...
    review_count = sum(
        cell["needs_review"] == "Yes"
        for cell in cells
    )

    header_text = (
        f"{title} | 70 square cells | "
        f"review cells: {review_count}"
    )
...
def create_excel_report(
    ...
    manifest_sheet.title = "Grid Manifest"
    ...
    manifest_values = [
        [
            ...
            row["status"],
            row["candidate_count"],
            row["direct_cell_count"],
            row["uncertain_cell_count"],
            row["uncertain_cell_ids"],
            row["square_side"],
            row["x_pitch"],
            row["y_pitch"],
            row["spacing_cv"],
            row["median_grid_error"],
            row["max_grid_error"],
            row["detection_scale"],
            row["notes"],
        ]
    ...
    for row_number in range(2, len(manifest_values) + 2):
        status_cell = manifest_sheet.cell(
            row=row_number,
            column=8,
        )

        if status_cell.value in {"PASS_AUTO", "PASS_MANUAL"}:
    ...
    readme_sheet["A1"] = "Third Trial Script 03 — Cell Grid Detection"
    readme_sheet["A2"] = (
        "This workbook contains D/RGB grid-detection results for the Third Trial."
    )
    readme_sheet["A3"] = (
        "The tray is treated as a fixed 7 x 10 = 70 cell lattice."
    )
    readme_sheet["A4"] = (
        "Square ownership zones are used so seedlings growing slightly outside cup rims are still assigned to the correct cell."
    )
    readme_sheet["A5"] = (
        "Green squares in overlays are directly supported by cup evidence. Yellow squares are fitted by the global lattice and should be reviewed. Blue squares are manual grids."
    )
    readme_sheet["A6"] = (
        "This script does not calculate germination, emergence, growth rate, NDVI or NDRE."
    )
    readme_sheet["A7"] = (
        "Day 7 bug-eaten plant adjustment will be handled later in the growth-rate comparison stage."
    )

    for row in range(1, 8):
        readme_sheet[f"A{row}"].alignment = Alignment(
            wrap_text=True,
            vertical="top",
        )
...
def save_settings(path: Path):
    ...
    settings = {
        "purpose": "Third Trial D/RGB 70-cell square grid detection",
        "input_root": str(INPUT_ROOT),
        "output_root": str(OUTPUT_ROOT),
        "rows": ROWS,
        "columns": COLS,
        "expected_cells": EXPECTED_CELLS,
        "square_zone_ratio": DEFAULT_SQUARE_ZONE_RATIO,
        "pass_min_direct_cells": PASS_MIN_DIRECT_CELLS,
        "pass_max_grid_error": PASS_MAX_GRID_ERROR,
        "check_min_direct_cells": CHECK_MIN_DIRECT_CELLS,
        "check_max_grid_error": CHECK_MAX_GRID_ERROR,
        "day7_bug_note": (
            "Day 7 may include bug-eaten plants. This grid-detection script "
            "does not adjust growth values. Later scripts should preserve both "
            "observed and adjusted/imputed Day 7 values."
        ),
    }
...
def process_one_job(
    ...
    overlay_path = (
        output_folder
        / f"{capture_id}_70_square_cell_grid_overlay.jpg"
    )
    ...
    if mode == "auto":
        ...
        points = get_manual_corner_centres(
            rgb,
            title=f"Manual grid | {day} | {tray} | {capture_id}",
        )

        if points is None:
            raise RuntimeError(
                "Manual grid was cancelled before four points were selected."
            )
    ...
    save_square_grid_overlay(
        ...
        cells,
        overlay_path,
        title=f"{day} | {tray} | {capture_id} | {status}",
    )
    ...
    manifest_row = {
        ...
        "direct_cell_count": metrics["direct_cell_count"],
        "uncertain_cell_count": metrics["uncertain_cell_count"],
        "uncertain_cell_ids": ";".join(
            str(value)
            for value in metrics["uncertain_cell_ids"]
        ),
        "square_side": metrics["square_side"],
        "x_pitch": metrics["x_pitch"],
        "y_pitch": metrics["y_pitch"],
        "spacing_cv": metrics["spacing_cv"],
        "median_grid_error": metrics["median_grid_error"],
        "max_grid_error": metrics["max_grid_error"],
        "detection_scale": metrics["detection_scale"],
        "notes": "",
    }
        ...
        coordinate_rows.append(
            {
                ...
                "grid_error": cell["grid_error"],
                "coordinate_source": cell["coordinate_source"],
                "needs_review": cell["needs_review"],
            }
        )

    return manifest_row, coordinate_rows, status


def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Script 03: Detect Third Trial D/RGB 7x10 square cell grid."
        )
    )
    ...
    parser.add_argument(
        ...
        choices=["auto", "manual"],
        default="auto",
        help="auto = Hough/lattice detection, manual = four clicked corner cells.",
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="List detected D/RGB crop jobs without processing.",
    )
    ...
    parser.add_argument(
        ...
        help="Temporary maximum dimension for auto detection.",
    )
    ...
    parser.add_argument(
        ...
        help="Square side length as fraction of smaller grid pitch.",
    )
    ...
    jobs = collect_jobs(
        days_filter=selected_days,
        trays_filter=selected_trays,
    )

    print("\nSCRIPT 03 — THIRD TRIAL CELL GRID DETECTION")
    print("=" * 70)
    ...
    if not jobs:
        print("No D/RGB cropped images were found.")
        return 1
    ...
    if args.dry_run:
        print("\nDry run complete. No grid outputs created.")
        return 0
    ...
    for job in jobs:
        ...
        try:
            manifest_row, cell_rows, status = process_one_job(
                job=job,
                mode=args.mode,
            ...
            if status == "SKIPPED_EXISTS":
                skipped_row = {
                    ...
                    "max_grid_error": "",
                    "detection_scale": "",
                    "notes": "Existing overlay found. Use --overwrite to reprocess.",
                }

                manifest_rows.append(skipped_row)
                status_counts["SKIPPED_EXISTS"] = status_counts.get("SKIPPED_EXISTS", 0) + 1
                print("SKIPPED_EXISTS")
                continue

            manifest_rows.append(manifest_row)
            coordinate_rows.extend(cell_rows)

            status_counts[status] = status_counts.get(status, 0) + 1

            print(
                f"{status} | direct={manifest_row['direct_cell_count']} | "
                f"review={manifest_row['uncertain_cell_count']} | "
                f"median_error={float(manifest_row['median_grid_error']):.4f}"
            )
    ...
    excel_path = (
        OUTPUT_ROOT
        / "_reports"
        / "cell_grid_report.xlsx"
    )

    config_path = (
        OUTPUT_ROOT
        / "_config"
        / "grid_detection_settings.json"
    )
    ...
    if status_counts.get("FAIL", 0) > 0:
        print(
            "\nSome images failed. Use manual mode for failed Day/Tray items."
        )
        return 1
